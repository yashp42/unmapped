...
async def list_lore(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100)):
    cursor = get_database().lore.find({"$or": [{"status": {"$exists": False}}, {"status": "published"}]}, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit)
    return await cursor.to_list(length=limit)


@router.get("/{lore_id}", response_model=LoreOut)
async def get_lore(lore_id: str, current_user: dict | None = Depends(get_optional_user)):
    ...
    if lore.get("status") not in (None, "published") and not (current_user and (current_user.get("id") == lore.get("user_id") or current_user.get("role") == "admin")):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lore entry not found")
    return lore

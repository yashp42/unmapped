...
async def list_tracks(skip: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100)):
    cursor = db.tracks.find({}, {"_id": 0}).sort("title", 1).skip(skip).limit(limit)
...
async def create_track(payload: TrackIn, current_user: dict = Depends(get_current_user)):
    ...
    return await db.tracks.find_one({"id": track["id"]}, {"_id": 0})
...
async def update_track(
    ...
    return await db.tracks.find_one({"id": track_id}, {"_id": 0})
